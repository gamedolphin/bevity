...
pub fn main() -> anyhow::Result<()> {
    println!("cargo:rerun-if-changed=src/exported.rs");
    println!("cargo:rerun-if-changed=build.rs");

    std::fs::create_dir_all("../Assets/bevity")?;

    let file = std::fs::read_to_string("src/exported.rs")?;
